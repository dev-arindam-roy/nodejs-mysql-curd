    ...
    req.getConnection((err, conn) => {
        ...
        conn.query(ckUser, (err, data) => {
            ...
            if (data.length > 0) {
                ...
                let query = "INSERT INTO `users`(`name`, `email`, `image`, `age`) VALUES('"+ name +"', '"+ email +"', '"+ profile_image +"', 0)";
                conn.query(query, (err, data) => {
                    if (err) {
                        return res.json(err);
                    }
                    return res.redirect('/add?add=ok');
                });
            }
        });
    });
    ...
    req.getConnection((err, conn) => {
        ...
        conn.query(ckUser, (err, data) => {
            ...
            if (data.length > 0) {
                ...
                let query = "UPDATE `users` SET `name` = '"+ name +"', `email` = '"+ email +"', `age` = '"+ age +"', `image` = '"+ profile_image +"' WHERE `id` = '"+ id +"'";
                conn.query(query, (err, data) => {
                    if (err) {
                        return res.json(err);
                    }
                    return res.redirect('/edit/'+ id +'?user=update');
                });
            }
        });
    });
